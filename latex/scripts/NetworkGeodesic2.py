...
class NetworkGeodesic:
    def __init__(self, p_= None) :
        ...
        self._sigma_pre = np.array(p_['sig_pre'])
        self._sigma_sel = np.array(p_['sig_sel'])              
        ...
        self._u_sel_u_pre_gain = 0.7
        self._u_sel_u_pre_gain = 1.0
        #self._u_sel_u_pre_gain = 2.0
        #self._u_sel_u_pre_gain = 2.0
        #self._u_sel_sig_l = 100.0
        self._u_sel_sig_l = 50.0
        self._nObjects = len(self._objects)

        # precomputed const 
        self._invSigma_pre, self._den_pre = self._ut.getMultiGaussianConst(self._sigma_pre)
        self._invSigma_sel, self._den_sel = self._ut.getMultiGaussianConst(self._sigma_sel)
        ...
        for o in self._objects:
            Wo.append(self._ut.multiGaussianPrecomp(self._ref, o, self._invSigma_pre, self._den_pre))
            # w = self._ut.multiGaussianPrecomp(self._ref, o, self._invSigma_pre, self._den_pre)
            # w /= w.sum()
            # Wo.append(w)
        if len(Wo) > 0:
        ...
        for i in range(self._N):
            w = self._ut.multiGaussianPrecomp(self._ref, self._ref[i,:], self._invSigma_pre, self._den_pre)
            w /= w.sum()
            self._W_pre[i,:] = w
            #self._W_pre[i,:] = self._ut.multiGaussianPrecomp(self._ref, self._ref[i,:], self._invSigma_pre, self._den_pre)            

        # weights for u_sel
        for i in range(self._N):            
            w = self._ut.multiGaussianPrecomp(self._ref, self._ref[i,:], self._invSigma_sel, self._den_sel)
            w /= w.max()
            self._W_sel[i,:] = w - 1.0
    ...
    def step(self, input_):
        ...
        if right > 0.0:
            ...
            w = self._ut.sigmoid(self._ref[:,1] - MU[1], 0.0, 50.0) * self._ut.multiGaussianPrecomp(MU, self._ref, self._invSigmaLR, self._denLR)
            #self._W_pre_right = w/w.sum()
            self._W_pre_right = w
            ...
            du_pre += right*self._W_pre_right
        ...
        if left > 0.0:
            ...
            for i in range(self._nObjects):
                MU += self._o[i]*self._objects[i]
            w = self._ut.sigmoid(MU[1] - self._ref[:,1],0.0, 50.0) * self._ut.multiGaussianPrecomp(MU, self._ref, self._invSigmaLR, self._denLR)
            #self._W_pre_left = w/w.sum()
            self._W_pre_left = w
        ...
        # test : s_sel_act = s_sel*0.0 + 0.001 
