...
class PostureTrackService(Service):
    def __init__(self, video_srv, motion_srv, params={}):
        ...
        self.useRobotCam = self.parameters['camSource'] == 'robot'

        self.video_srv = video_srv
        self.motion_srv = motion_srv

        #self.humanBaseFramePos = params['humanBaseFramePos']

        if self.useRobotCam:    

            self.cameraFrame = "CameraTop"

            # Image size VGA
            #imgShape = (480,640, 3)
            #imgShape = (240,320, 3)
        
            imgShape = (self.parameters['imgHeight'],self.parameters['imgWidth'], self.parameters['imgColorDim'])
            imgSize = np.prod(imgShape)
            
            resolution = vision_definitions.kQVGA
            colorSpace = vision_definitions.kRGBColorSpace
            #fps = 10
            fps = self.parameters['imgFPSRobot']
            camID = 0 # top camera
    ...
    def step(self):
        ...
        # transform = self.motion_srv.getTransform(self.cameraFrame, self.frame, True)
